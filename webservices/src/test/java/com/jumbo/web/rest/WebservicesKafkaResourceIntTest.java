...
import com.jumbo.config.KafkaProperties;
...
class WebservicesKafkaResourceIntTest {
  ...
  void setup() {
    ...
    WebservicesKafkaResource kafkaResource = new WebservicesKafkaResource(kafkaProperties);
  ...
  void producesMessages() throws Exception {
    restMockMvc
      .perform(post("/api/webservices-kafka/publish/topic-produce?message=value-produce"))
  ...
  void consumesMessages() throws Exception {
    ...
    MvcResult mvcResult = restMockMvc
      .perform(get("/api/webservices-kafka/consume?topic=topic-consume"))
      .andExpect(status().isOk())
